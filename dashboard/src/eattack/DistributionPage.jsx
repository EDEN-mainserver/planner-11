    ...
    <div className="p-6 space-y-5">
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
        ...
        <ol className="text-xs text-amber-600 space-y-1 list-decimal list-inside">
          ...
          <li>Vendor ID: Wing URL의 vendorId 파라미터 값</li>
        </ol>
      </div>
    </div>
  );
                ...
                <tbody>
                  {data.items.map((item, i) => (
                    <tr key={i} className={`border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                      <td className="px-3 py-2.5 text-gray-700 font-medium max-w-[160px] truncate" title={item.item_name}>{item.item_name}</td>
                      ...
                      <td className="px-3 py-2.5 text-right">
                        {item.has_cost
                          ? <Badge color={item.margin >= 15 ? 'green' : item.margin >= 0 ? 'amber' : 'red'}>{item.margin}%</Badge>
...
// ── 유통 메인 ─────────────────────────────────────────────────
const TABS = [
  { key: 'coupang', label: '쿠팡', icon: '🛒', gradient: 'from-orange-400 to-red-500', description: '실시간 매출·순수익 대시보드' },
];
