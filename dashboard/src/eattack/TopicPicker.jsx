...
function ConvertPanel({ post, onConfirm, onBack }) {
  ...
  return (
    <div className="flex flex-col gap-3 h-full overflow-y-auto">
      {/* 뒤로 */}
      <button onClick={onBack} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 transition-colors w-fit flex-shrink-0">
      ...
      <div className="flex flex-col gap-2 flex-1 min-h-0">
        {converted && (
          <p className="text-[10px] text-violet-600 font-medium flex-shrink-0">✨ 변환 완료 — 수정 후 사용하기를 누르세요</p>
        )}
        <div className="flex flex-col gap-1 flex-shrink-0">
          <label className="text-[10px] text-gray-500 font-medium">제목</label>
          <input
            type="text"
            value={editTitle}
            onChange={e => setEditTitle(e.target.value)}
            placeholder={converted ? "" : "변환하면 자동 생성됩니다"}
            disabled={!converted}
            className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-violet-400 transition-colors disabled:bg-gray-50 disabled:text-gray-400 disabled:placeholder-gray-300"
          />
        </div>
        <div className="flex flex-col gap-1 flex-1 min-h-0">
          <label className="text-[10px] text-gray-500 font-medium">썰 스크립트</label>
          <textarea
            value={editText}
            onChange={e => setEditText(e.target.value)}
            className="flex-1 w-full px-3 py-2 text-xs border border-gray-200 rounded-lg outline-none focus:border-violet-400 transition-colors resize-none min-h-[80px]"
          />
        </div>
      </div>

      {error && <p className="text-xs text-red-500 px-1 flex-shrink-0">{error}</p>}
        ...
        {converted ? (
          <>
            <button
              ...
              disabled={converting}
              className="flex items-center justify-center gap-1 px-3 py-2 text-xs font-medium rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 transition-all disabled:opacity-50 flex-shrink-0"
            >
              {converting ? "변환 중..." : "재변환"}
            </button>
            <button
              onClick={() => onConfirm({ title: editTitle, text: editText })}
              disabled={!editText.trim()}
              className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-violet-600 text-white hover:bg-violet-700 transition-all disabled:opacity-50"
            >
              사용하기
            </button>
          </>
          ...
          <>
            <button
              onClick={() => onConfirm({ title: "", text: rawText })}
