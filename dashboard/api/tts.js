...
// ── Voices 핸들러 (GET ?_fn=voices) ─────────────────────────────────────────
async function handleVoices(res) {
  const apiKey = process.env.ELEVENLABS_API_KEY;
  if (!apiKey) return res.status(500).json({ error: "ELEVENLABS_API_KEY 환경변수가 설정되지 않았습니다." });

  const elevenRes = await fetch("https://api.elevenlabs.io/v1/voices", { headers: { "xi-api-key": apiKey } });
  if (!elevenRes.ok) return res.status(elevenRes.status).json({ error: `ElevenLabs 오류 (${elevenRes.status})` });

  const data = await elevenRes.json();
  const GENDER_KO  = { male: "남성", female: "여성" };
  const ACCENT_KO  = { american: "미국식", british: "영국식", australian: "호주식", irish: "아일랜드식", african: "아프리카식", indian: "인도식" };
  const AGE_KO     = { young: "젊음", "middle aged": "중간", old: "노년" };
  const USECASE_KO = { narration: "내레이션", news: "뉴스", meditation: "명상", "social media": "SNS", conversational: "대화형", characters: "캐릭터", audiobook: "오디오북" };

  const voices = (data.voices ?? []).map(v => {
    const lb = v.labels ?? {};
    const tags = [GENDER_KO[lb.gender], ACCENT_KO[lb.accent], AGE_KO[lb.age], USECASE_KO[lb.use_case]].filter(Boolean);
    return { id: v.voice_id, name: v.name, desc: tags.join(" · ") || lb.description || "" };
  });
...
export default async function handler(req, res) {
  // GET ?_fn=voices → 보이스 목록 반환
  if (req.method === "GET" && req.query._fn === "voices") {
