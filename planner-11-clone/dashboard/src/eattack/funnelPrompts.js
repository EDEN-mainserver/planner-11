...
// 레퍼런스 블로그 글 분석 지침 생성
function buildRefGuide(refBlog) {
  if (!refBlog || !refBlog.trim()) return '';
  return `
## 레퍼런스 블로그 글 (말투·흐름 참고)
아래 글의 말투, 문장 호흡, 도입부 방식, 단락 전환 흐름을 최대한 따라하세요.
단, 내용과 주제는 위의 상품/서비스 정보에 맞게 새로 작성하세요.
 ...
 */
export function buildTofuPrompt({ productName, benefits, target, conversionGoal, platform, refBlog }) {
...
플랫폼: ${pg.name}
${buildRefGuide(refBlog)}
TOFU 단계 목표: 잠재 고객이 아직 당신의 상품을 모릅니다. 타겟의 일상적인 불편함이나 욕구를 건드려서 "맞아, 나 이런 문제 있었는데"라는 공감을 먼저 끌어내세요. 상품 홍보는 최소화하고 정보성 콘텐츠로 접근하세요.

작성 지침:
- 어조: ${pg.tone}
- 형식: ${pg.format}
- 글 구조: 후킹 제목 → 공감 도입부 → 문제 상황 묘사 → 문제의 원인 분석 → 해결 방향 제시 → 자연스러운 CTA
- SEO 제목: 타겟이 검색할 법한 키워드를 포함한 제목
- CTA: 부드럽게 더 알아보도록 유도 (강매 금지)
- 절대 금지: ##, ###, **, *, >, --- 등 마크다운 기호 사용 금지. 소제목은 그냥 텍스트로만 쓰세요.

다음 JSON 형식으로만 출력하세요. 다른 텍스트 없이:
{
  "title": "SEO 최적화 제목",
  "sections": [
    { "heading": "소제목(없으면 null)", "content": "본문 내용 (마크다운 기호 없이 순수 텍스트)" }
  ],
  "cta": "CTA 문구",
 ...
 */
export function buildMofuPrompt({ productName, benefits, target, conversionGoal, platform, refBlog }) {
  const pg = PLATFORM_GUIDE[platform] || PLATFORM_GUIDE.naver;
...
플랫폼: ${pg.name}
${buildRefGuide(refBlog)}
MOFU 단계 목표: 잠재 고객이 이미 해결책을 찾고 있으며 여러 옵션을 비교 중입니다. ${productName}의 차별화된 강점을 구체적인 근거(수치, 사례, 비교)로 설득하세요. "왜 이걸 선택해야 하는가?"에 명확히 답하는 글이어야 합니다.

작성 지침:
- 어조: ${pg.tone}
- 형식: ${pg.format}
- 글 구조: 후킹 제목 → 독자 상황 공감 → 기존 방법의 한계 → ${productName} 차별점 3가지 → 실제 효과/사례 → 강력한 CTA
- 핵심 혜택: ${benefits.map((b, i) => `혜택${i + 1}: ${b}`).join(' / ')}
 ...
 */
export function buildBofuPrompt({ productName, benefits, target, conversionGoal, platform, refBlog }) {
  const pg = PLATFORM_GUIDE[platform] || PLATFORM_GUIDE.naver;
...
플랫폼: ${pg.name}
${buildRefGuide(refBlog)}
BOFU 단계 목표: 잠재 고객이 구매/신청을 거의 결심했지만 망설이고 있습니다. 마지막 불안/의심을 해소하고 "지금 당장 해야 하는 이유"를 만들어주세요. 사회적 증거(후기, 사례), 희소성, 보장 등을 활용하세요.
