            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {brands.map(b => (
                ...
                >
                  ...
                  <span className={`text-xs font-semibold truncate ${selectedBrandId === b.id ? 'text-pink-700' : 'text-gray-700'}`}>
                    {b.name}
                  </span>
                  <button
                    onClick={e => { e.stopPropagation(); deleteBrand(b.id); }}
                    className="absolute top-1 right-1 w-4 h-4 rounded-full bg-gray-100 text-gray-400 hover:bg-red-100 hover:text-red-500 text-[10px] hidden group-hover:flex items-center justify-center transition-colors"
                  >×</button>
                </div>
              ))}
            </div>
